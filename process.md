...
学习者判断：如果没有 mask，`<PAD>` 可能会被当作输入提供给大模型；并主动指出自己还不理解张量、Attention 中的关注权重、softmax 和 Loss。当前选择先单独学习张量。
...
- 张量的基础解释与验收
- Attention 关注权重、softmax 和 Loss 的基础解释与验收
...
继续保持小步讲解，先完成张量，再依次补齐 Attention 前置概念，之后确认 `<PAD>` 和 mask，最后做第 01 课验收。暂不进入词向量 3D 可视化对应的第 02 课，也不提前归档第 01 课。
